...
In chapter 9 we took a brief look at the LWP::Simple module. Here is
...
mentioned briefly in chapter 9, this describes version 3.x of
...
module repository for use with ActivePerl is still a 2.x version.1
...
* $text = $parser->get\_trimmed\_text($endtag) Works in the same way as the get_text method, except that any sequences of white space characters are collapsed to a single space, and any leading or trailing white space is removed.
...
* $parser->implicit\_body\_p\_tag($boolean) If the boolean value is true, the parser will force there to be a `<p>` element surrounding any elements which should not be immediately contained within a `<body>` tag.
...
* $parser->ignore\_ignorable\_whitespace($boolean) If the boolean value is true then the parser will not build nodes for white space which can be ignored without affecting the structure of the document.
...
* $parser->store\_comments($boolean), $parser->store\_declarations($boolean), $parser->store\_pis($boolean) These control whether or not comments, declarations, and processing instructions are stored in the parser tree.
